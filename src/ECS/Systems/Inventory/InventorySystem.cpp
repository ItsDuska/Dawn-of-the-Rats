...
void InventorySystem::addNewItem(Coordinator& entityManager, AssetManager& assetManager)
{
	for (auto& entity : this->mEntities)
	{
		...
		inventory._items.push_back(std::make_shared<Item>("InventoryItems",
			sf::IntRect(48, 16, 16, 16), this->background.getGlobalBounds(),assetManager));
	}
}
