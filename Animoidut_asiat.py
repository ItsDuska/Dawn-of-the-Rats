...
class AnimatedObj(pygame.sprite.Sprite):
    def __init__(self,pos,maxRuudut,kuva,näyttö,width,height,kasvi,animate,type) -> None:
        ...
        self.image = pygame.image.load(os.path.join("Kuvat","Palikat","Animoidut_palikat",self.folder,self.kuva[0])).convert_alpha() 
    ...
    def AddParticle(self,sus):
        if random.randint(1,35) != 1:
            return
        x, y = (random.randint(-sus[0]+self.rect.x,-sus[0]+self.rect.x+30),random.randint(-sus[1]+self.rect.y+20,-sus[1]+self.rect.y+30))
